...
begin
	using DifferentiationInterface  # the backend-agnostic interface
	using ForwardDiff: ForwardDiff  # a backend implementation
end

# ╔═╡ f073c6f9-a400-4735-a088-c5d6077940fa
using BenchmarkTools

# ╔═╡ 16966dca-15fa-4f00-a3ee-bb5815db959f
using Plots

# ╔═╡ 202c1487-0d6b-4d9a-a80e-9d0089884563
using Optimization

# ╔═╡ 2c097d44-46f2-40a2-a702-2ffe6c409554
using OptimizationOptimJL

# ╔═╡ e9716cc0-fd55-4575-a679-9171fd5ba50f
using JuMP, Ipopt

# ╔═╡ e6c78d10-afa1-4248-8811-6c677ef7efbd
using Unitful
...
md"""
Let's consider some applications of AD.
"""
...
md"""
Let's consider the [Newton's method](https://en.wikipedia.org/wiki/Newton's_method) as a trivial application for automatic differentiation.

The root ``x^*`` of a function
...
```
for a sufficiently good starting point ``x_0``.
Let's consider calculating the square root of ``a > 0`` as the root ``x^*`` of
```math
	f(x) = x^2 - a.
```
Of course, in this trivial example we could use the symbolic derivative
```math
	f'(x) = 2x,
```
but let's use the automatic derivative instead.
"""

# ╔═╡ 0ea739f2-906e-46dc-8ade-2bb39a4b24cc
const c::Float64 = π   # some example a

# ╔═╡ a12d9f5c-3650-4f1d-81ec-600c90ac6639
f(x) = x^2 - c

# ╔═╡ 3620bfdf-3b0a-4bd2-a667-c43fb898f5f7
function newton(f, x0::AbstractFloat; backend=AutoForwardDiff())
	x = x0 - /(value_and_derivative(f, backend, x0)...)
	xprev = typeof(x)(x0)
	# WARNING: if it doesn't converge it is an endless loop
	while abs(x - xprev) > 2eps(x0)
		xprev = x
		x = x - /(value_and_derivative(f, backend, x)...)
	end
	return x
end
...
md"""
We got the right result. So let's see how efficient we are.
"""

# ╔═╡ 874d66bd-f03d-4b98-98a5-d9fba63fd5a8
tn = @benchmark newton(f, c)

# ╔═╡ fdb4160e-1595-45b0-a5af-9317b1233730
t0 = @benchmark sqrt(c)

# ╔═╡ f67c4004-e45d-4826-b2ed-a124d6a6fd7f
md"""
Nice, we just need $(round(median(tn.times) / median(t0.times), digits=1)) times the time for the analytic evaluation of this function (for this start value).

Let's also compare the analytic evaluation of the derivative:
"""
...
# ╔═╡ f9a61b6f-d720-4610-9b76-caf1cf6f05ce
tan = @benchmark analytic_newton(c)
...
rosenbrock(x::Number, y::Number, a=1.0, b=100.0) = (a - x)^2 + b*(y - x^2)^2

# ╔═╡ 7faaeef5-e64c-4e0c-9d89-ebd8ac2b34bf
md"""
Let's visualize (the logarithm) of the function to get an idea.
"""

# ╔═╡ 8958eebb-5cf7-40a8-b9e8-05451de7ca0c
xs = range(-2, 2, length=501)

# ╔═╡ fd415998-ffca-4129-97b6-6907cb0d3aa2
ys = range(-1, 3, length=501)

# ╔═╡ df4b3d61-a53d-4153-a2ea-bcb2f2b7e007
md"""
And a 3D view of the function itself, in case it helps (use `plotly()` for interactive plots).
"""
...
md"""
We'll use the `Optimization` package, to find the solution.
The `Optimization` package is a meta-package, providing a uniform interface to many optimization solvers.
First, we need to add a vectorized method, accepting an input vector ``\boldsymbol{x}`` and a parameter vector ``\boldsymbol{p}``:
"""

# ╔═╡ 6d2303e2-fe30-46d4-8e39-50b3201ba555
rosenbrock(x, p) = rosenbrock(x[1], x[2], p[1], p[2])

# ╔═╡ 4f35aaf2-143e-47ef-8beb-1ca746d35db5
heatmap(xs, ys, log10 ∘ rosenbrock)

# ╔═╡ 15710d1c-3ccb-4c67-a341-4d7fceac59ac
contourf(xs, ys, log10 ∘ rosenbrock)

# ╔═╡ 55e4ce73-9f03-4783-907a-ac44452c49ea
begin
	downsampling = length(xs) ÷ 20
	wireframe(
		xs[begin:downsampling:end], ys[begin:downsampling:end], rosenbrock
	)
	surface!(xs, ys, rosenbrock, alpha=0.9)
	xlabel!("x")
	ylabel!("y")
end

# ╔═╡ 8271f378-b387-401c-b3a7-450aa4906e16
md"""
We define the optimization function:
"""

# ╔═╡ ba94abf4-7175-423b-be42-0f143ee45a20
...
# ╔═╡ 91560560-6bee-488b-a1b7-44ff6e85ca3e
_p = [1.0, 100.0]

# ╔═╡ 9f9e1ad6-f2ca-45b4-b5f0-979e58622694
md"""
And a starting point:
"""

# ╔═╡ 0026dc3d-7499-4a61-adaa-ea83a49526f4
x0 = [0.0, 0.0]

# ╔═╡ cb29dc32-6b0c-4539-961a-18bf9a54b0ab
md"""
This allows us to specify the full problem to optimize:
"""

# ╔═╡ bfdb9e53-e253-423f-a523-932ba7dccf60
...
# ╔═╡ 0840ccb0-8561-4d2c-b4f2-d92855d5fc97
md"""
#### Derivative-free solution
"""
...
# ╔═╡ c875eec7-e20f-4d82-863f-3af1a4be2ea4
sol.retcode
...
md"""
We found a solution in $(sol.stats.iterations) iterations, which took us $(round(sol.stats.time, sigdigits=2)) seconds and $(sol.stats.fevals) function evaluations.

We can extract further information by investigating the output of the underlying solver:
"""

# ╔═╡ d066cbc6-c72d-4b62-a055-672ce12a00bc
sol.original

# ╔═╡ cf6bbc74-1c06-4d36-af82-28ad797a418e
md"""
...
# ╔═╡ 1a4408de-866e-4ea7-b681-e12079240afa
optf_ad = OptimizationFunction(rosenbrock, Optimization.AutoForwardDiff())

# ╔═╡ 5c4e7835-b343-437b-9233-c2488a5ac534
prob_ad = OptimizationProblem(optf_ad, x0, _p)

# ╔═╡ ab33af96-2e5a-4ed2-8120-426bf1c52518
md"""
Let's try using the [Broyden–Fletcher–Goldfarb–Shanno algorithm](https://en.wikipedia.org/wiki/Broyden%E2%80%93Fletcher%E2%80%93Goldfarb%E2%80%93Shanno_algorithm).
"""

# ╔═╡ e67ebda5-06ca-4862-8312-ad3715f30ad5
sol_bfgs = solve(prob_ad, Optim.BFGS())

# ╔═╡ 54afe9ee-bd14-4563-83a9-20cade562871
sol_bfgs.retcode

# ╔═╡ 214201a9-352d-4a6d-8b64-6deec39bc130
sol_bfgs.u
...
md"""
#### Second-order Hessian based-solver
"""
...
# ╔═╡ 8472ac34-8465-4bb0-ac55-4c3dc055d76f
sol_newt.original
...
I'll use `Ipopt` as it is a nonlinear solver.
"""

# ╔═╡ 48c03cde-5273-445e-983e-f33c678cea4a
begin
	begin
		model = Model(Ipopt.Optimizer)
		@variable(model, u[1:2])  # we again choose a vectorized approach
		p = [1, 100]
		@objective(model, Min, (p[1] -u[1])^2 + p[2]*(u[2] - u[1]^2)^2)
	end
	optimize!(model)
	model
end

# ╔═╡ 0a0f5e6a-f5bf-4c16-875c-e2019d70f178
is_solved_and_feasible(model)

# ╔═╡ d5fa5857-3fe0-4bda-8a10-ffac14dc9307
...
# ╔═╡ a1899ccb-4fc8-4a37-961b-798348806c0a
md"""
# Sensitivity analysis

AD allows us to trace derivatives, and thus Jacobians, through complicated algorithms.
For example, we can determine the Jacobian of the solution of a differential equation with respect to the input parameters.

Let's consider the Lotka-Volterra equations as a simple example:
```math
	\begin{pmatrix}
		\frac{du_1}{dt}
		\\ \frac{du_2}{dt}
...
# ╔═╡ 4bd4d17a-f1cb-4288-b568-152edd344ee4
import DifferentialEquations as DE

# ╔═╡ b6ce5972-3afc-409a-9675-fef6ea7cb86f
...
# ╔═╡ a0b9a1f7-318a-4ae0-a289-01d9d4c0293d
md"""
We use the `DifferentialEquations` package to solve the equation.
Let's first solve and visualize it to get an impression:
"""

# ╔═╡ 7c838570-9c36-40cb-93e1-e21cd391b194
lv_solution = DE.solve(
	DE.ODEProblem(lotka_volterra!, [1.0, 1.0], (0.0u"s", 10.0u"s"), [1.5, 1.0, 3.0, 1.0]u"1/s"),
	DE.Tsit5()
)

# ╔═╡ 20074c88-ffdd-41d4-ad8a-141c56e0f849
plot(lv_solution)

# ╔═╡ 52cc9bad-0c28-43b6-8434-a1180565c130
function lotka_volterra_solution(p)
	prob = DE.ODEProblem(
		lotka_volterra!, eltype(p).([1.0, 1.0]), (0.0, 10.0), p
	)
	return DE.solve(prob, DE.Tsit5(), save_everystep=false)[end]	
end

# ╔═╡ 81a01052-9c0f-4003-b169-cb9071c11882
md"""
Let's consider the solution at `t=10`.
...
# ╔═╡ 53ff969c-1be0-4d28-a8da-372edb7f0464
jacobian(lotka_volterra_solution, AutoForwardDiff(), [1.5, 1.0, 3.0, 1.0])

# ╔═╡ 43fefdf9-5a4b-468a-a2bd-906c446329dc
TableOfContents()

# ╔═╡ 33550471-baa0-4ba6-b28c-2873b5ec0132
@Symbolics.variables x, y, a, b;

# ╔═╡ 9561712e-d1cb-4c34-bcc7-cbc60e30c256
Markdown.parse(
"""
Let's move to some more intricate problems.
We try to find the (global) minimum of the Rosenbrock function.
...
git-tree-sha1 = "86d710bfa6e86d301c59f9ca7b5cfffa649f57a9"
...
git-tree-sha1 = "006238a52d94ee8618c9f905fbabb177d37e4db0"
...
deps = ["ADTypes", "Adapt", "ArrayInterface", "DataStructures", "DiffEqBase", "DiffEqNoiseProcess", "DocStringExtensions", "FastPower", "FiniteDiff", "ForwardDiff", "JumpProcesses", "LevyArea", "LinearAlgebra", "Logging", "MuladdMacro", "OrdinaryDiffEqCore", "OrdinaryDiffEqDifferentiation", "OrdinaryDiffEqNonlinearSolve", "Random", "RecursiveArrayTools", "Reexport", "SciMLBase", "SciMLOperators", "SimpleNonlinearSolve", "SparseArrays", "StaticArrays", "UnPack"]
git-tree-sha1 = "a4f362cce06506d1be3a6cdd50b4828c8a0d3a7b"
