...
md"""
## The most naive way[^1]

[^1]: See version 2 of the notebook for a more idiomatic version using type promotion.
This reduces a lot of boilerplate.
"""

# ╔═╡ cac73c9a-0a18-4c1d-87eb-b54bb991415b
struct DualV1{T <: Number}
	x::T
	dx::T
end

# ╔═╡ 81ed17a9-d183-47b0-a4c7-593346ebf05e
derivative(x::DualV1) = x.dx
...
begin
	import Base: +
	+(a::DualV1, b::DualV1) = DualV1(a.x + b.x, a.dx + b.dx)
	+(a::DualV1, b::Number) = DualV1(a.x + b, a.dx)
	+(a::Number, b::DualV1) = +(b, a)
end
...
# ╔═╡ 8d3c29cb-b500-4d0e-a3ba-bb4d605ec961
begin
	import Base: *
	*(a::DualV1, b::DualV1) = DualV1(a.x * b.x, a.dx*b.x + a.x*b.dx)
	*(a::DualV1, b::Number) = DualV1(a.x * b, a.dx*b)
	*(a::Number, b::DualV1) = *(b, a)
end
...
# ╔═╡ f6535b6e-bf0b-4fab-8954-49dc0356b232
begin
	import Base: -
	-(a::DualV1, b::DualV1) = a + -1*b
	-(a::DualV1, b::Number) = a + -1*b
	-(a::Number, b::DualV1) = a + -1*b
end
...
begin
	import Base: ^
	^(a::DualV1, b::DualV1) = DualV1(a.x^b.x, b * a.x^(b-1) * a.dx + a^b.x * log(a) * b.dx)
	^(a::DualV1, b::Number) = DualV1(a.x^b, b * a.x^(b-1) * a.dx)
	^(a::Number, b::DualV1) = DualV1(a^b.x, a^b.x * log(a) * b.dx)
end
...
begin
	import Base: /
	/(a::DualV1, b::DualV1) = a * b^-1
	/(a::DualV1, b::Number) = a * b^-1
...
begin
	import Base: zero
	zero(a::DualV1) = DualV1(zero(a.x), zero(a.dx))
end

# ╔═╡ b13c0fd9-009e-44be-a328-06ab432a6b33
begin
	import Base: one
	one(a::DualV1) = DualV1(one(a.x), zero(a.dx))
end
...
begin
	import Base: abs
	abs(a::DualV1) = DualV1(abs(a.x), a.x*a.dx/abs(a.x))
end
...
# ╔═╡ c6c8ffee-1d44-44a1-8ada-ee36863a043c
md"""
...
# ╔═╡ 00941c6a-f56d-44a3-a07f-34fab8168be8
begin
	import Base: promote_rule, convert
	# numbers get promoted to dual numbers
	promote_rule(::Type{Dual}, ::Type{Number}) = Dual
	# converting a number to a dual, we get 0 as derivative
